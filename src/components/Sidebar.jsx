    ...
    const SIDEBAR_ITEMS = [
        {
            ...
            icon: <MdDashboard />,
            href: "/",
            roles: [USER_ROLES.SUPER_ADMIN, USER_ROLES.MARKETING_ADMIN, USER_ROLES.MERCHANT]
        },
        {
            name: "Product Order",
            icon: <BsCartFill />,
        ...
        },
        {
            ...
            roles: [USER_ROLES.SUPER_ADMIN, USER_ROLES.MARKETING_ADMIN]
        },
        {
                ...
                <div className={`transition-all duration-300 z-10 bg-website-light h-[95.75%] ml-5 shadow-sm flex flex-col pb-4 rounded-lg`}>
                    ...
                    <div className="flex flex-row items-center mx-8 gap-16 my-8">
                        <div className={`flex flex-row gap-1 flex-1`}>
                            <img
                                src={open ? unleash_logo : unleash_icon}
                                alt="Unleash Logo"
                            ...
                            <div className={`px-2 py-1 rounded-full bg-unleash-blue mt-auto h-fit justify-center items-center flex ${open ? "" : "hidden"}`}>
                                <p className="text-[10px] font-montserrat text-white font-medium">
                                    {ROLES_CONFIGS[accountType]?.tag.name}
                                </p>
                            </div>
                        </div>
                    ...
                    >
                        ...
                        <p className={`text-sm transition-all duration-100 ease-in-out ${open ? "w-full ms-4" : "w-0 text-nowrap overflow-hidden"}`}>
                            Logout
                        </p>
                    </div>
...
function SidebarItem({ currentLocation, icon, name, href, open }) {
    ...
    return (
        <div className={`flex flex-row items-center cursor-pointer rounded-lg hover:brightness-95
            ...
            <p className={`font-poppins text-sm select-none ${open ? "w-full ms-3" : "hidden"}`}>
                {name}
            </p>
        </div>
    );
